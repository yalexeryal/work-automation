...
def create_synopses_dict(synopses: list) -> dict:
    ...
    synopses_dict = {}
    for synopsis in synopses:
        flag_syn = "Утверждён" in synopsis[9]
        # if synopsis[2] == 'Программирование' and flag_syn:
        if  flag_syn:
            synopsis_date = date_in_date(synopsis[0])
            synopsis_time = synopsis[1]
            moduls = synopsis[2]
            moduls = moduls.upper()
            ...
            task = synopsis[4]
            expert = synopsis[5]
            synopsis_str_shot = f"{synopsis_date}  {synopsis_time}   {task}  @{expert} \n"
            ...
            print(synopsis[9], synopsis[2], synopsis[0], synopsis[1])
